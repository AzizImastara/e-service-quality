...
@st.cache_resource
def load_artifact(pkl_path: str = "svm_kfold_final.pkl"):
...
def predict_texts(artifact, texts: list[str]) -> list[str]:
    ...
    label_encoder = artifact["label_encoder"]  # gunakan encoder

    X = vectorizer.transform(texts)
    preds = model.predict(X)
    labels = label_encoder.inverse_transform(preds)  # ubah index → label string
...
def render_csv_page(artifact):
    ...
    with st.expander("Panduan pengguna", expanded=True):
        st.markdown(
            "- Siapkan file CSV dengan kolom `cleaned_content` berisi teks yang akan diklasifikasi.\n"
            "- Anda dapat mengunduh template, isi datanya, lalu unggah kembali.\n"
            "- Klik 'Proses data' untuk mendapatkan hasil klasifikasi."
        )

    # Template CSV
    template_df = pd.DataFrame({"cleaned_content": [
        "contoh teks pertama",
        "contoh teks kedua",
    ]})
    ...
    if df is not None:
        ...
        if "cleaned_content" not in df.columns:
            st.warning("Kolom 'cleaned_content' tidak ditemukan di CSV.")
            return

        if st.button("Proses data", type="primary"):
            texts = df["cleaned_content"].astype(str).fillna("").tolist()
...
def main():
    ...
    artifact = load_artifact("svm_kfold_final.pkl")
